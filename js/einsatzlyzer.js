    ...
    const initOverviewMaps = (scope = document) => {
        scope.querySelectorAll('[data-map-toggle]:not([data-map-bound])').forEach((toggle) => {
            ...
            toggle.addEventListener('click', async () => {
                ...
                if (label) label.textContent = open ? (fflEinsatzlyzer.showMapText || 'Einsatzorte auf Karte anzeigen') : (fflEinsatzlyzer.closeMapText || 'Karte wieder schließen');
                if (subtitle) subtitle.textContent = open ? (toggle.dataset.mapClosedSubtitle || fflEinsatzlyzer.discoverMapText || 'Einsatzorte auf der Karte entdecken') : (fflEinsatzlyzer.backReportsText || 'Zurück zu den Einsatzberichten');
                toggle.setAttribute('aria-label', open ? (fflEinsatzlyzer.showMapText || 'Einsatzorte auf Karte anzeigen') : (fflEinsatzlyzer.closeMapText || 'Karte wieder schließen'));
                if (open) return;
    ...
    const initShare = () => {
        document.querySelectorAll('[data-share-url]:not([data-share-bound])').forEach((button) => {
            ...
            button.addEventListener('click', async () => {
                ...
                await copyText(url);
                showTemporaryLabel(button, fflEinsatzlyzer.copyText || 'Link kopiert');
            });
        });

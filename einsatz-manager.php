...
define( 'FFL_EINSATZLYZER_VERSION', '10.6.14' );
...
", $manual_vehicle_lines ) );
        ...
        <section class="ffl-admin-panel ffl-admin-panel--wide">
            <div class="ffl-admin-panel__heading"><span class="dashicons dashicons-admin-links"></span><div><h3><?php echo esc_html( ffl_lang( 'Weiterführende Links', 'Further links' ) ); ?></h3><p><?php echo esc_html( ffl_lang( 'Zum Beispiel Presseberichte oder Berichte anderer Feuerwehren.', 'For example press coverage or reports from other fire departments.' ) ); ?></p></div></div>
            <div class="ffl-link-fields">
                <?php for ( $i = 1; $i <= 5; $i++ ) : ?>
                    <div class="ffl-link-field-row">
                        <label><span><?php echo esc_html( sprintf( ffl_lang( 'Link %d', 'Link %d' ), $i ) ); ?></span><input type="url" name="ffl_link_<?php echo esc_attr( $i ); ?>" value="<?php echo esc_url( get_post_meta( $post->ID, '_ffl_link_' . $i, true ) ); ?>" placeholder="https://"></label>
                        <label><span><?php echo esc_html( ffl_lang( 'Quellangabe', 'Source' ) ); ?></span><input type="text" name="ffl_link_source_<?php echo esc_attr( $i ); ?>" value="<?php echo esc_attr( get_post_meta( $post->ID, '_ffl_link_source_' . $i, true ) ); ?>" placeholder="<?php echo esc_attr( ffl_lang( 'z. B. Feuerwehr Uplengen oder Presseportal', 'e.g. Uplengen Fire Department or press portal' ) ); ?>"></label>
                    </div>
                <?php endfor; ?>
            </div>
        </section>
...
", $vehicle_labels ) );
    ...
    for ( $i = 1; $i <= 5; $i++ ) {
        $field        = 'ffl_link_' . $i;
        $source_field = 'ffl_link_source_' . $i;
        if ( isset( $_POST[ $field ] ) ) {
            update_post_meta( $post_id, '_' . $field, esc_url_raw( wp_unslash( $_POST[ $field ] ) ) );
        }
        if ( isset( $_POST[ $source_field ] ) ) {
            update_post_meta( $post_id, '_' . $source_field, sanitize_text_field( wp_unslash( $_POST[ $source_field ] ) ) );
        }
    }
...
function ffl_get_einsatz_distance( $post_id, $coords = null ) {
    ...
    if ( in_array( $mode, array( 'road', 'road_time', 'air_road_time' ), true ) ) {
        $parts[] = $prefix . ( $route['road_km'] < 0.1 ? ffl_lang( 'unter 0,1 km Fahrstrecke', 'under 0.1 km driving distance' ) : number_format_i18n( $route['road_km'], 1 ) . ' km ' . ffl_lang( 'Fahrstrecke', 'driving distance' ) );
    }
    if ( in_array( $mode, array( 'time', 'road_time', 'air_road_time' ), true ) ) {
        $parts[] = $prefix . ( $route['duration_min'] <= 0 ? ffl_lang( 'unter 1 Min. Fahrzeit', 'under 1 min driving time' ) : (int) $route['duration_min'] . ' ' . ffl_lang( 'Min. Fahrzeit', 'min driving time' ) );
    }

...
const FFL_IMPEX_SCHEMA_VERSION = 1;
...
function ffl_impex_export_entry( $post_id, $options, &$zip_files, &$checksums, &$warnings ) {
    ...
    foreach ( $image_map as $attachment_id => $roles ) {
        ...
        foreach ( $attachment_meta as $meta_key => $meta_values ) {
            if ( '_wp_attachment_image_alt' === $meta_key || 0 === strpos( $meta_key, '_ffl_' ) ) {
...
function ffl_impex_validate_and_extract( $zip_path, $token ) {
    ...
    for ( $i = 0; $i < $zip->numFiles; $i++ ) {
        ...
        $allowed = in_array( $name, array( 'manifest.json', 'einsaetze.json', 'pruefsummen.json' ), true );
        if ( 0 === strpos( $name, 'bilder/' ) ) {
...
function ffl_impex_import_attachment( $image, $post_id, $dir, &$state ) {
    ...
    if ( ! empty( $image['meta'] ) && is_array( $image['meta'] ) ) {
        foreach ( $image['meta'] as $key => $values ) {
            if ( '_wp_attachment_image_alt' === $key || 0 === strpos( $key, '_ffl_' ) ) {
...
function ffl_impex_import_entry( $entry, &$state ) {
    ...
    $post_data = array(
        ...
        'post_content'   => wp_kses_post( isset( $entry['post']['content'] ) ? $entry['post']['content'] : '' ),
        'post_excerpt'   => sanitize_textarea_field( isset( $entry['post']['excerpt'] ) ? $entry['post']['excerpt'] : '' ),
        'post_status'    => $status,
...
function ffl_impex_handle_import_batch() {
    ...
    if ( 'running' === $state['status'] && $state['position'] >= count( $entries ) ) {
        ...
        ffl_impex_add_log( $state, 'success', 'Import abgeschlossen.' );
...
function ffl_impex_render_admin_page() {
    ...
    <div class="wrap ffl-impex-wrap">
        <h1>Einsätze sichern, übertragen und wiederherstellen</h1>
        <p class="ffl-impex-lead">Die vollständige ZIP-Sicherung enthält Einsatzdaten, Beitragsbilder, Galerien, eingebettete Bilder, Bildtexte und Prüfsummen. CSV ist zusätzlich für Excel-Auswertungen gedacht.</p>
            ...
            <div class="ffl-impex-grid">
                <section class="ffl-impex-card ffl-impex-card--export">
                    ...
                    <div>
                        ...
                        <p>Erzeugt eine eigenständige ZIP-Datei mit JSON-Daten und den echten Originalbildern.</p>
                    </div>

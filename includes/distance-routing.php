...
function ffl_get_saved_route_data( $post_id, $station = null, $coords = null ) {
    ...
    return array(
        ...
        'duration_min'  => max( 1, (int) round( (float) $duration_min ) ),
...
function ffl_route_admin_status_html( $post_id ) {
    ...
    <div class="ffl-single-route-status__grid">
      ...
      <div><span><?php echo esc_html( ffl_lang( 'Fahrstrecke', 'Driving distance' ) ); ?></span><strong><?php echo $saved ? esc_html( number_format_i18n( $saved['road_km'], 1 ) . ' km' ) : '—'; ?></strong></div>
      <div><span><?php echo esc_html( ffl_lang( 'Fahrzeit', 'Driving time' ) ); ?></span><strong><?php echo $saved ? esc_html( sprintf( ffl_lang( 'ca. %d Minuten', 'about %d minutes' ), $saved['duration_min'] ) ) : '—'; ?></strong></div>
    </div>
    <?php if ( $saved ) : ?>
...
function ffl_render_distance_settings_modal() {
    ...
    (function(){
      ...
      function render(){progress.max=Math.max(1,state.ids.length);progress.value=state.index;text.textContent=`${state.index} / ${state.ids.length} ${i18n.processed} · ${state.success} ${i18n.success} · ${state.errors.length} ${i18n.errors}`;if(state.errors.length){errorsBox.hidden=false;errorsBox.innerHTML='<strong>'+i18n.errors+':</strong><ul>'+state.errors.map(e=>'<li>'+e+'</li>').join('')+'</ul>';}else errorsBox.hidden=true;}
      function next(){if(!state.running||state.paused||state.stopped)return;if(state.index>=state.ids.length){state.running=false;current.textContent='';render();return;}const id=state.ids[state.index];current.textContent='#'+id+' …';request({action:'ffl_route_calculate',nonce,post_id:id,force:state.force?1:0}).then(res=>{if(res.success)state.success++;else state.errors.push('#'+id+' '+((res.data&&res.data.message)||i18n.genericError));}).catch(()=>state.errors.push('#'+id+' '+i18n.networkError)).finally(()=>{state.index++;render();setTimeout(next,1200);});}
